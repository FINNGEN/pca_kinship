...
def finn_or_not(args):
    ...
    if not os.path.isfile(args.finngen_eur_outliers) or args.force:
        ...
        if len(euros) == 0:
            ...
            # create Lapland population group
            laps,lap = [],eur_outlier_path + 'lap.txt'
            ...
            for entry in [(eur,euros),(fin,finns),(finngen,finngens),(lap,laps)]:
            ...
            for tag,f in [('finngen',finngen),('eur',eur),('fin',fin),('lap',lap)]:
...
def project_lapland(args):
    # get people born in lapland
    # mark outliers 
    return 
